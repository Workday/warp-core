...
class SmartNumberArbiterSpec extends WarpJUnitSpec with CorePersistenceAware with WarpLogging {
  ...
  def usesLongSlidingWindow(): Unit = {
    ...
    val incomingTestExecution: TestExecutionRowLike = persistDummyTestExecution(testID, 400)
  ...
  /**
    * Checks that `vote` returns [[None]] when sliding window size is too small.
    */
  @UnitTest
  def illegalState(): Unit = {
    ...
    arbiter.vote(new Ballot(testId), incomingTestExecution) should be (empty)
  }
