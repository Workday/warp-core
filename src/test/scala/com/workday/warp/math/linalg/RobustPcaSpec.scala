...
class RobustPcaSpec extends WarpJUnitSpec {
  ...
  def testRsvd(): Unit = { // X:
    ...
    val X: Matrix = this.reshape(timeSeries, 7, 9)
    ...
    val E_matrix_r: Matrix = this.reshape(E_r, 7, 9)
    val S_matrix_r: Matrix = this.reshape(S_r, 7, 9)
    val L_matrix_r: Matrix = this.reshape(L_r, 7, 9)
