...
Furthermore, settings.gradle is ignored during dynamic GradleBuild invocations, so our attempt to avert this conflict
by setting rootProject.name does not take effect for the recursed task graph, only the originally invoked task graph.
